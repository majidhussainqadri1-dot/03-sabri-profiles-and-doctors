...
final class SPD_Activator {
	...
	private static function migrate_public_ids() {
		$offset = 0;
		do {
			$users = get_users( array( 'fields' => 'ids', 'number' => 200, 'offset' => $offset, 'orderby' => 'ID', 'order' => 'ASC' ) );
			foreach ( $users as $user_id ) {
				SPD_Public_Identity::ensure( $user_id );
				if ( ! get_user_meta( $user_id, SPD_Profile_Policy::META_VERSION, true ) ) {
					update_user_meta( $user_id, SPD_Profile_Policy::META_VERSION, 1 );
				}
			}
			$offset += count( $users );
	...
	private static function schedule_retention() {
		if ( ! wp_next_scheduled( 'spd_legacy_audit_retention' ) ) { wp_schedule_event( time() + HOUR_IN_SECONDS, 'daily', 'spd_legacy_audit_retention' ); }
	}
	...
	private static function pages() {
		...
		$definitions = array(
			...
			'profile' => array( 'Member Profile Compatibility', '[sabri_member_profile]', 'member-profile' ),
			'edit' => array( 'Account Profile', '[sabri_edit_profile]', 'account-profile' ),
		);
	...
	private static function managed_page( $key, $title, $shortcode, $slug, $stored_id ) {
		if ( $stored_id ) {
			...
			if ( $page instanceof WP_Post && 'page' === $page->post_type && $key === get_post_meta( $stored_id, '_spd_managed_page_key', true ) ) {
				if ( trim( $page->post_content ) !== $shortcode ) { wp_update_post( array( 'ID' => $stored_id, 'post_content' => $shortcode ) ); }
				return $stored_id;
			}
		}
		...
		if ( $slug_page instanceof WP_Post ) {
			...
			if ( $is_owned || $is_exact ) { update_post_meta( $slug_page->ID, '_spd_managed_page_key', $key ); return absint( $slug_page->ID ); }
			$slug .= '-spd';
		}
		$id = wp_insert_post( array( 'post_title' => $title, 'post_name' => wp_unique_post_slug( $slug, 0, 'publish', 'page', 0 ), 'post_content' => $shortcode, 'post_status' => 'publish', 'post_type' => 'page' ) );
	...
	private static function migrate_verified_snapshots() {
		...
		foreach ( $users as $user_id ) { if ( ! SPD_Verification_Adapter::snapshot( $user_id ) ) { SPD_Verification_Adapter::capture_snapshot( $user_id ); } }
	}
}
