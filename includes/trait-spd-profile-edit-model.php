...
trait SPD_Profile_Edit_Model {
	public function edit_model( $user_id ) {
		$profile = $this->find_by_user_id( $user_id );
		if ( is_wp_error( $profile ) ) {
			return $profile;
		}
		if ( ! SPD_Authorization::can_edit_profile( $profile, $user_id ) ) {
		...
		return array(
			...
			'completeness'     => $this->completeness( $profile, SPD_Membership_Adapter::claims( $user_id ), SPD_Verification_Adapter::approved_fields( $user_id ) ),
			'professional'     => SPD_Verification_Adapter::approved_fields( $user_id ),
		);
	}
