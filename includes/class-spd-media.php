...
final class SPD_Media {
	...
	public static function prepare_upload( $user_id, $field, $purpose, array $context=array() ) {
		...
		try {
			...
			do_action( 'sabri_file24_profile_provider_failure', array( 'owner' => 'file03', 'provider' => 'profile_media_scan', 'surface' => 'media_upload_scan', 'exception_class' => sanitize_key( get_class( $exception ) ), 'at' => SPD_Helpers::now() ) );
			return new WP_Error('spd_scan_unavailable',__( 'The image safety scanner is temporarily unavailable.','sabri-profiles-doctors'),array('status'=>503));
		}
