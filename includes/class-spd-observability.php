...
final class SPD_Observability {
	...
	public function cron_schedules( $schedules ) {
		$schedules['spd_five_minutes'] = array(
			'interval' => 300,
			'display'  => __( 'Every five minutes', 'sabri-profiles-doctors' ),
		);
		return $schedules;
	}
	...
	public static function set_safe_mode( $enabled, $reason = '' ) {
		...
		if ( '' === $reason ) {
			return new WP_Error( 'spd_safe_mode_reason_required', __( 'A safe-mode reason is required.', 'sabri-profiles-doctors' ) );
		}
	...
	public function dispatch_outbox() {
		...
		foreach ( $ids as $id ) {
			...
			if ( ! is_array( $payload ) ) {
				$this->fail_outbox_claim( $row, $token, $attempts, 'invalid_payload' );
				continue;
			}
			try {
				...
				$wpdb->update(
					...
					array( 'id' => absint( $id ), 'lease_token' => $token )
				);
			} catch ( Throwable $exception ) {
				$this->fail_outbox_claim( $row, $token, $attempts, sanitize_key( get_class( $exception ) ) );
			}
		}
	...
	private function fail_outbox_claim( array $row, $token, $attempts, $error_code ) {
		...
		$wpdb->update(
			...
			array( 'id' => absint( $row['id'] ), 'lease_token' => $token )
		);
	}
	...
	public function migrate_profiles_batch() {
		...
		try {
			...
			foreach ( $users as $user_id ) {
				...
				if ( $failure && 'dead' === $failure['status'] ) {
					$cursor = $user_id;
			...
			if ( count( $users ) < 100 ) {
				...
				if ( ! $retry && ! $dead ) { update_option( 'spd_migration_completed_at', SPD_Helpers::now(), false ); }
				else { delete_option( 'spd_migration_completed_at' ); }
			}
		} finally {
	...
	private function record_migration_failure( $user_id, WP_Error $error ) {
		...
		$query  = $wpdb->prepare(
			"INSERT INTO {$table} (user_id,error_code,detail_hash,attempts,status,next_attempt_at,last_attempt_at) VALUES (%d,%s,%s,%d,%s,%s,%s) ON DUPLICATE KEY UPDATE error_code=VALUES(error_code),detail_hash=VALUES(detail_hash),attempts=VALUES(attempts),status=VALUES(status),next_attempt_at=VALUES(next_attempt_at),last_attempt_at=VALUES(last_attempt_at)",
			absint( $user_id ), sanitize_key( $error->get_error_code() ), $hash, $count, $status, gmdate( 'Y-m-d H:i:s', time() + $delay ), $now
		);
		$wpdb->query( $query ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
		if ( 'dead' === $status ) {
			do_action( 'sabri_file24_migration_quarantined', array( 'owner' => 'file03', 'user_id' => absint( $user_id ), 'error_code' => sanitize_key( $error->get_error_code() ), 'attempts' => $count ) );
		}
		return $status;
	}
	...
	private function clear_migration_failure( $user_id ) {
		global $wpdb;
		$wpdb->delete( SPD_DB::table( 'migration_failures' ), array( 'user_id' => absint( $user_id ) ) );
	}
	...
	public static function requeue_migration_user( $user_id, $actor_id, $reason ) {
		...
		$table = SPD_DB::table( 'migration_failures' );
		$updated = $wpdb->query( $wpdb->prepare( "UPDATE {$table} SET status='retry',attempts=0,next_attempt_at=UTC_TIMESTAMP(),last_attempt_at=UTC_TIMESTAMP() WHERE user_id=%d AND status='dead'", $user_id ) ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
	...
	private function migrate_legacy_projection( $user_id ) {
		...
		// values remain with File 00 and are still blocked for minors/unverified channels.
	...
	public function process_media_deletions() {
		SPD_Media::reconcile_storage_privacy( 100 );
		SPD_Media::process_deletion_queue( 50 );
	}
	...
	public function retention_cleanup() {
		...
		$wpdb->query( "UPDATE {$reports} SET reporter_user_id=0,details='',decision_note='',dedupe_hash=SHA2(CONCAT(report_uuid,':retained'),256) WHERE reporter_user_id<>0 AND status IN ('closed','rejected') AND updated_at<(UTC_TIMESTAMP()-INTERVAL 365 DAY)" ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
		$wpdb->query( "DELETE FROM {$events} WHERE status='delivered' AND created_at<(UTC_TIMESTAMP()-INTERVAL 730 DAY) AND event_name NOT IN ('ProfileTombstoned.v1','ProfileReported.v1','ProfileReportReviewed.v1')" ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
	...
	public static function repair( $execute = false ) {
		...
		if ( $execute && $plan ) {
			...
			if ( get_option( 'spd_reconciliation_required', false ) ) {
				if ( function_exists( 'wp_cache_flush_group' ) ) { wp_cache_flush_group( 'spd' ); }
				else { update_option( 'spd_profile_cache_generation', SPD_Profile_Repository::cache_generation() + 1, false ); }
