...
trait SPD_Frontend_Future {
	public function future_profile_sections( array $dto, $is_owner = false ) {
		...
		$future_state_version = 0; $freshness_versions = array();
		if ( $is_owner && ! empty( $dto['public_id'] ) ) {
			$native_profile = SPD_Profile_Repository::instance()->find_by_public_id_strict( (string) $dto['public_id'] );
			if ( is_array( $native_profile ) && ! empty( $native_profile['id'] ) ) {
				$state = spd_read_future_profile_state( absint( $native_profile['id'] ) );
				if ( is_array( $state ) ) { $future_state_version = absint( $state['version'] ?? 1 ); }
				global $wpdb;
				foreach ( array_keys( (array) ( $f['freshness'] ?? array() ) ) as $field_key ) {
					$wpdb->last_error = '';
					$value = $wpdb->get_var( $wpdb->prepare( 'SELECT version FROM ' . SPD_Future_Profile::attestations_table() . ' WHERE profile_id=%d AND field_key=%s LIMIT 1', absint( $native_profile['id'] ), sanitize_key( $field_key ) ) ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
					if ( ! $wpdb->last_error ) { $freshness_versions[ $field_key ] = null === $value ? 0 : absint( $value ); }
				}
			}
		}
		ob_start();
		...
		<?php $editions = (array) ( $f['multilingual_editions'] ?? array() ); if ( $editions ) : ?>
			<section class="spd-card"><h2><?php esc_html_e( 'Approved Language Editions', 'sabri-profiles-doctors' ); ?></h2><?php foreach ( $editions as $edition ) : ?><article lang="<?php echo esc_attr( $edition['locale'] ); ?>" data-spd-edition-locale="<?php echo esc_attr( strtolower( $edition['locale'] ) ); ?>" data-spd-edition-version="<?php echo esc_attr( absint( $edition['version'] ?? 0 ) ); ?>"><h3><?php echo esc_html( $edition['locale'] ); ?><?php echo 'machine' === $edition['source'] ? ' — ' . esc_html__( 'Machine translated, owner approved', 'sabri-profiles-doctors' ) : ''; ?></h3><?php if ( $edition['headline'] ) : ?><p><strong><?php echo esc_html( $edition['headline'] ); ?></strong></p><?php endif; ?><?php if ( $edition['bio'] ) : ?><p><?php echo nl2br( esc_html( $edition['bio'] ) ); ?></p><?php endif; ?></article><?php endforeach; ?></section>
		<?php endif; ?>
