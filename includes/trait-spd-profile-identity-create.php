...
trait SPD_Profile_Identity_Create {
	public function ensure_for_user( $user_id ) {
		...
		$table = SPD_DB::table( 'profiles' ); $row = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$table} WHERE user_id=%d LIMIT 1", $user_id ), ARRAY_A ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
		...
		if ( $row ) { $fields_table = SPD_DB::table( 'fields' ); $custom_slug_locked = '1' === (string) $wpdb->get_var( $wpdb->prepare( "SELECT field_value FROM {$fields_table} WHERE profile_id=%d AND field_key='custom_slug_locked' LIMIT 1", absint( $row['id'] ) ) ); } // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
		...
		if ( ! $row ) {
			...
			$result = SPD_DB::transaction( function() use ( $wpdb, $table, $user_id, $public_id, $slug, $type, $state, $locale, $claims, $now ) {
				if ( 'founder' === $type ) { $existing = $wpdb->get_var( "SELECT id FROM {$table} WHERE profile_type='founder' LIMIT 1" ); if ( $existing ) { return new WP_Error( 'spd_founder_profile_exists', __( 'An official Founder profile already exists.', 'sabri-profiles-doctors' ), array( 'status' => 409 ) ); } }
				...
				if ( ! $ok ) { $race = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$table} WHERE user_id=%d LIMIT 1", $user_id ), ARRAY_A ); return $race ? $race : new WP_Error( 'spd_profile_create_failed', __( 'The profile could not be created.', 'sabri-profiles-doctors' ) ); }
	...
	private function unique_slug( $base, $profile_id = 0 ) { global $wpdb; $table = SPD_DB::table( 'slugs' ); $candidate = $base; $counter = 2; while ( true ) { $owner = $wpdb->get_var( $wpdb->prepare( "SELECT profile_id FROM {$table} WHERE slug=%s LIMIT 1", $candidate ) ); if ( ! $owner || absint( $owner ) === absint( $profile_id ) ) { return $candidate; } $candidate = substr( $base, 0, 150 ) . '-' . $counter; $counter++; if ( $counter > 10000 ) { return substr( $base, 0, 120 ) . '-' . substr( hash( 'sha256', $base . ':' . $profile_id ), 0, 12 ); } } } // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
	private function record_slug( $profile_id, $slug, $current ) { global $wpdb; $table = SPD_DB::table( 'slugs' ); $slug = sanitize_title( $slug ); $existing = $wpdb->get_row( $wpdb->prepare( "SELECT id,profile_id FROM {$table} WHERE slug=%s LIMIT 1", $slug ), ARRAY_A ); if ( $existing && absint( $existing['profile_id'] ) !== absint( $profile_id ) ) { return new WP_Error( 'spd_slug_collision', __( 'The requested profile address is already reserved.', 'sabri-profiles-doctors' ), array( 'status' => 409 ) ); } if ( $current && false === $wpdb->update( $table, array( 'is_current' => 0 ), array( 'profile_id' => absint( $profile_id ) ) ) ) { return new WP_Error( 'spd_slug_history_failed', __( 'Profile address history could not be updated.', 'sabri-profiles-doctors' ) ); } if ( $existing ) { $ok = $wpdb->update( $table, array( 'is_current' => $current ? 1 : 0 ), array( 'id' => absint( $existing['id'] ) ) ); return false === $ok ? new WP_Error( 'spd_slug_update_failed', __( 'The profile address could not be updated.', 'sabri-profiles-doctors' ) ) : true; } $ok = $wpdb->insert( $table, array( 'profile_id' => absint( $profile_id ), 'slug' => $slug, 'is_current' => $current ? 1 : 0, 'created_at' => SPD_Helpers::now() ) ); return $ok ? true : new WP_Error( 'spd_slug_insert_failed', __( 'The profile address could not be recorded.', 'sabri-profiles-doctors' ) ); }
