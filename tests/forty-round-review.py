...
check(9, 'Idempotency key bounds', 'strlen( $key ) > 200' in events and 'Idempotency-Key' in rest, 'oversized keys fail safely')
check(10, 'Deterministic replay', 'return $committed_response;' in update and "'profile' => $after" not in update, 'first response equals replay response')
...
check(19, 'URL credentials rejected', "isset( $target['user'] ) || isset( $target['pass'] )" in helpers, 'credential-bearing URLs are rejected')
check(20, 'Provider health freshness', 'current_contract_claim( $provider_health' in timeline and '300 )' in timeline, 'timeline health is current/versioned')
...
check(22, 'Provider result bound', 'MAX_PROVIDER_ITEMS' in timeline and 'count( $result ) > self::MAX_PROVIDER_ITEMS' in timeline, 'oversized provider result opens circuit')
