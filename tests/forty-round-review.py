...
release_match = re.search(r"define\( 'SPD_VERSION', '1\.2\.0-rc(\d+)' \)", main)
release_ok = bool(release_match and int(release_match.group(1)) >= 9)
check(1, 'Release identity', release_ok and "SPD_CONTRACT_VERSION', '1.4.0'" in main and 'FUTURE-SUPERSET-18' in main and '80-ROUND-CORRECTIVE-REVIEW' in main and 'THIRD-TEN-ROUND-CORRECTIVE-REVIEW' in main and 'FOURTH-TEN-ROUND-CORRECTIVE-REVIEW' in main and 'FIFTH-TEN-ROUND-CORRECTIVE-REVIEW' in main and 'SIXTH-TEN-ROUND-CORRECTIVE-REVIEW' in main and 'SEVENTH-TEN-ROUND-CORRECTIVE-REVIEW' in main and 'EIGHTH-TEN-ROUND-CORRECTIVE-REVIEW' in main and 'NINTH-TEN-ROUND-CORRECTIVE-REVIEW' in main, 'rc9-or-later identity, contract 1.4.0, future plan identity and preserved corrective review markers are aligned')
...
readme_match = re.search(r'^Stable tag: 1\.2\.0-rc(\d+)$', readme, flags=re.M)
check(39, 'Package identity', bool(readme_match and int(readme_match.group(1)) >= 9) and "SPD_DB_VERSION', '1.2.0'" in main and "SPD_CONTRACT_VERSION', '1.4.0'" in main, 'software/contract/schema identities remain explicit for rc9-or-later release candidates')
check(40, 'Truthful completion boundary', 'repository candidate' in readme.lower() and 'hostinger staging' in readme.lower() and 'live deployment' in readme.lower() and 'exact deployed code is unverified' in readme.lower(), 'repository completion does not claim staging/live/deployed truth')
