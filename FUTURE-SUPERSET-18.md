...
Candidate: `1.2.0-rc1`  
Contract: `1.4.0`  
Plan identity: `SSH-F03-PLAN-2026-v1.0 + 2026-08-07 central addendum + FUTURE-SUPERSET-18`  
...
| F03-FUT-02 | Selective-Disclosure Professional Card | HMAC-signed, expiring, scope-limited disclosure packet; max 24h; revoked by existing share epoch; REST create/read routes | Only already-public-safe scopes can be disclosed; no raw evidence/private contacts; invalid/expired/revoked token fails closed |
...
| F03-FUT-08 | Grounded “Ask About This Doctor’s Work” AI | Member-only question route; File 16 grounded provider; same-origin citations; explicit medical-scope block | File 16 owns AI execution; diagnosis/prescription/dose/emergency/cure requests rejected; unavailable provider => 503, no local hallucinated fallback |
...
| F03-FUT-16 | Professional Legacy / Memorial State | Additive `spd_profile_future_state`; active/retired/legacy; governed legacy approval; non-active state suppresses direct contact/appointment | Legacy requires Founder/admin governance; content may remain as scholarly record but active-service representation stops |
...
| F03-FUT-18 | Federation-Ready Public Profile Projection | Explicit opt-in actor projection; actual inbox/outbox accepted only from current external federation transport provider | File 03 owns profile projection, not ActivityPub/federation transport; opt-out or provider failure => transport inactive |
...
Future-profile mutations are owner/governor authorized and use the existing File 03 idempotency store through explicit `Idempotency-Key` replay protection. They emit auditable outbox events. Selective disclosure is stateless, signed, bounded and revocable. Translation, freshness and future-state writes are additive/retry-safe and cannot grant doctor verification, publishing, ranking, appointment or clinical authority.
