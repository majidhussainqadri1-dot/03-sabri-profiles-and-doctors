  ...
  document.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      if (form.matches('[data-spd-disclosure]')) {
        ...
        const data = await request('me/disclosures', 'POST', { scopes, ttl: Math.round(hours * 3600) });
      ...
      if (form.matches('[data-spd-translation]')) {
        event.preventDefault();
        const fd = new FormData(form);
        await request('me/translations', 'POST', { locale: fd.get('locale'), headline: fd.get('headline'), bio: fd.get('bio'), source: fd.get('source') });
      ...
      if (form.matches('[data-spd-reconfirm]')) {
        event.preventDefault();
        const fd = new FormData(form);
        const data = await request('me/reconfirm', 'POST', { field_key: fd.get('field_key'), days: Number(fd.get('days') || 365) });
      ...
      if (form.matches('[data-spd-future-state]')) {
        ...
        const fd = new FormData(form);
        const id = form.dataset.publicId;
        const data = await request(`profiles/${encodeURIComponent(id)}/future-state`, 'PUT', { professional_lifecycle: fd.get('professional_lifecycle'), lifecycle_reason: fd.get('lifecycle_reason'), federation_opt_in: fd.get('federation_opt_in') === '1' });
